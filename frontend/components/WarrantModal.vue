  ...
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/70">
    <div class="w-full max-w-lg bg-slate-950 border border-slate-700 rounded-2xl p-5 shadow-xl space-y-4">
      <header class="flex items-start justify-between gap-4">
        ...
        <button class="text-xs text-slate-400 hover:text-slate-200" @click="$emit('close')">
          ✕
        </button>
      </header>

      <div class="space-y-2 max-h-64 overflow-y-auto pr-1">
        <label v-for="s in suspects" :key="s.id"
          ...
          <input type="radio" class="mt-1 text-emerald-500" name="suspect" :value="s.id" v-model="selectedId" />
      ...
      <footer class="flex items-center justify-end gap-2">
        <button class="px-3 py-1.5 text-xs rounded-lg border border-slate-600 text-slate-200 hover:bg-slate-800"
          @click="$emit('close')">
          Cancelar
        </button>
        <button
          ...
          :disabled="!selectedId || loading" @click="onConfirm">
          <span v-if="loading">Enviando...</span>
          <span v-else>Emitir mandado</span>
        </button>
      </footer>

      <div v-if="resultMessage" class="text-xs mt-1" :class="resultSuccess ? 'text-emerald-300' : 'text-red-300'">
        {{ resultMessage }}
      </div>
    </div>
  </div>
