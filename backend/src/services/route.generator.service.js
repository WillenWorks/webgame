 ...
 */
export async function generateRouteService({ activeCaseId, steps = 5, optionsPerStep = null }) {
  if (!activeCaseId) {
  ...
  const [[caseRow]] = await pool.execute(`SELECT profile_id FROM active_cases WHERE id = ?`, [activeCaseId]);
  let decoyTarget = 3; // padrão base
  if (caseRow?.profile_id) {
    const [[pRow]] = await pool.execute(`SELECT xp, rank_id FROM profiles WHERE id = ?`, [caseRow.profile_id]);
    const [ranks] = await pool.execute(`SELECT id, min_xp, difficulty_modifier FROM ranks ORDER BY id ASC`);
  ...
  for (let i = 0; i < route.length; i++) {
    ...
    if (i < route.length - 1) {
      ...
      const decoys = pickDecoys(cities, exclude, Math.max(0, effectiveOptionsPerStep - 1));
